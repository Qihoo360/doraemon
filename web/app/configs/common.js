...
export function removeObjectUseless(obj, key, list = []) {
  if (!(list instanceof Array)) {
    throw Error('list must be array')
  }
  if (obj[key] == undefined || obj[key] == null || obj[key] === '' || (list.indexOf(obj[key]) > -1)) {
    delete obj[key]
  }
  return obj
}
...
// 异步请求需要走redux的方式
export const createAjaxAction = (createdApi, startAction, endAction, rejectAction) => (request = {}, resolve, reject, config) => (dispatch) => {
  ...
  const _reject = (error) => {
    ...
    if (rejectAction) dispatch(rejectAction({ req: request, res: error }))
